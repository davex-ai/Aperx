...
public class LeaveService {
    ...
    public LeaveRequestResponse reviewRequest(Long reviewerUserId, Long leaveRequestId, LeaveReviewRequest request) {
        ...
        if (!leaveRequest.getEmployee().getCompany().getId().equals(reviewer.getCompany().getId())) {
        ...
        if (!isAdmin && !isDirectManager) {
            throw new BadRequestException("You can only review leave requests for your direct reports");
        }
    ...
    public List<LeaveRequestResponse> getPendingForReviewer(Long companyId, Long reviewerUserId, boolean isAdmin) {
        if (isAdmin) {
            return leaveRequestRepository.findByEmployeeCompanyIdAndStatusOrderByCreatedAtDesc(companyId, LeaveStatus.PENDING).stream()
                    .map(this::toResponse)
                    .collect(Collectors.toList());
        }
        ...
        Employee manager = employeeRepository.findByUserId(reviewerUserId)
                .orElseThrow(() -> new ResourceNotFoundException("Employee profile not found"));
        return leaveRequestRepository.findByEmployeeManagerIdAndStatus(manager.getId(), LeaveStatus.PENDING).stream()
                .map(this::toResponse)
                .collect(Collectors.toList());
    }
