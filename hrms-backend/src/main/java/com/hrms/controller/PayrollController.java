...
public class PayrollController {
    ...
    public ResponseEntity<PayrollPreviewResponse> previewPayroll(@Valid @RequestBody PayrollRunRequest request) {
        return ResponseEntity.ok(payrollService.previewPayroll(authUtil.getCurrentCompanyId(), request));
    }

    @PostMapping("/run")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<List<PayslipResponse>> runPayroll(@Valid @RequestBody PayrollRunRequest request) {
        return ResponseEntity.ok(payrollService.runPayroll(authUtil.getCurrentCompanyId(), authUtil.getCurrentUserId(), request));
    }
    ...
    public ResponseEntity<Resource> downloadPayslip(@PathVariable Long id) {
        ...
        Payslip payslip = payrollService.getPayslipForDownload(authUtil.getCurrentCompanyId(), id, authUtil.getCurrentUserId(), privileged);
